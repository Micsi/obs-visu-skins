// Test helpers for walking Vue VNode trees produced by the terminal renderers,
// plus minimal Tokens/Ctx stubs matching the @obs/visu-contract surface.
// Mirrors packages/skins/ionic/tests/_vnode.ts (same harness shape).
...
/** All `data-arg` strings present anywhere in the tree. */
export function args(node: unknown): string[] {
  return flatten(node)
    .map((v) => v.props?.["data-arg"])
    .filter((a): a is string => typeof a === "string");
}
